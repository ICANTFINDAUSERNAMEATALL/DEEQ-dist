...
def evaluate_model01(agent, data, window_size, debug, ma_state_size):
    agent.i2 = []
    agent.inventory = []
    iternum = 0
    total_profit = 0
    ...
    for j in range(data_length):
        state0, state1 = get_state(data, j, window_size + 1, 0, norm_mas_lst[j], 0)
        sols.append(state0)
    ...
    for i in range(5):
        ...
        for k in range(1):
            slls = []
            for j in range(data_length):
                state1 = get_invs_state(0, k) # 
                slls.append(state1)
            action_json[i][k] = agent._act(sols, slls, is_eval=True)

    for t in tqdm(range(data_length)):
        ### ### ### ### ### ### ### ###
        # Updating the Inventory/CD ###
        ### ### ### ### ### ### ### ###
        rlinv = []
        rli2 = []
        for i in range(len(agent.i2)):
            cnum = agent.i2[i][1]
            if cnum == 0:
                rlinv.append(agent.inventory[i])
                rli2.append(agent.i2[i])
            else:
                agent.i2[i][1] -= 1
        for i in rlinv:
        ...
        candnum += 1
        reward = 0
        state_lst.append((state0, state1))

        ### ### ### ### ### ### ### ### ### ### ###
        # Select an action and calculate reward ###
        ### ### ### ### ### ### ### ### ### ### ###
        min_ddc = min(min_ddc, ddcnt)
        dl = [min_ddc - ddcnt, 0, 4] #  + dd_to_apply_penalty_start
        dl.sort()
        
        if len(agent.i2) >= 4:
            action = 0
        else:
            action = action_json[min(len(agent.i2), 4)][dl[1]][t]
        ...
        if action == 1:
            buys += 1
            bsl.append("BUY")
            agent.inventory.append(["LONG", data[t][3], data[t][3] - sl_amount, data[t][3] + tp_amount])
            reward, cd = calc_tp_sl(t + 1, data, data_length, "LONG", data[t][3] - sl_amount, data[t][3] + tp_amount)
            total_profit += reward
            if reward > 0:
                wins += 1
        ...
        # SELL
            ...
            sells += 1
            bsl.append("SELL")
            agent.inventory.append(["SHORT", data[t][3], data[t][3] + sl_amount, data[t][3] - tp_amount])
            reward, cd = calc_tp_sl(t + 1, data, data_length, "SHORT", data[t][3] + sl_amount + eval_slippage, data[t][3] - tp_amount - eval_slippage)
            total_profit += reward
            if reward > 0:
                wins += 1
            ...
            agent.i2.append(["SHORT", cd])

            history.append((data[t], "SELL"))
            if debug:
                pass
                # logging.debug("Sell at: {} | Position: {}".format(format_currency(data[t][3])))
        # HOLD
        else:
            reward = 0
            history.append((data[t], "HOLD"))
        ...
        next_state0, next_state1 = get_state(data, t + 1, window_size + 1, 0, norm_mas_lst[t + 1], dl[1], 0)
        next_state_lst.append((next_state0, next_state1))
...
def evaluate_model01_1(agents, data, window_size, debug, ma_state_size):
    iternum = 0
    total_profit = 0
    data_length = len(data) - 1
    ...
    for agent in agents:
        ...
        for i in range(invs_gen_len):
            ...
            all_action_json[i].append(agent._act2(sols, slls, is_eval=True))
    ...
    """
    nl = {}
    for inv_state in all_action_json.keys():
        nl[inv_state] = []
        for outter_list_index in range(len(all_action_json[inv_state])):
            ddl = [0 for i in range(len(all_action_json[inv_state][outter_list_index]))]
            for inner_index in range(len(all_action_json[inv_state][outter_list_index])):
                ddl[inner_index] += all_action_json[inv_state][outter_list_index][inner_index] # is a np.array
        nl[inv_state] = ddl
    
    for inv_state in nl.keys():
        for i in range(len(nl[inv_state])):
            nl[inv_state][i] = np.argmax(nl[inv_state][i])
    ...
    for t in tqdm(range(data_length)):
        ...
        if len(agent.i2) >= invs_gen_len - 1:
            action = 0
        else:
            action = action_json[min(len(agent.i2), invs_gen_len - 1)][t]
